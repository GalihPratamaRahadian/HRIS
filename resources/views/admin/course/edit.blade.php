			...
			<div class="card-body">
				...
				<form id="mainForm">
					...
					<div class="form-group">
						...
						<input type="text" name="course_title" class="form-control" placeholder="Judul Course" value="{{ $course->course_title }}" required>
					...
					<div class="form-group">
						...
						<select class="form-control" name="pass_requirement" required>
...
<script type="text/javascript">
	$(function(){
		...
		$form.find(`[name="id_department"]`).val(`{{ $course->id_department }}`).trigger('change')

		$form.find(`[name="id_employee_group"]`).select2({
			'placeholder': '- Pilih Grup Karyawan',
		})
		$form.find(`[name="id_employee_group"]`).val(`{{ $course->id_employee_group }}`).trigger('change')
		...
		$form.find(`[name="video_source"]`).val(`{{ $course->video_source }}`).trigger('change')
		$form.find(`[name="is_published"]`).val(`{{ $course->is_published }}`).trigger('change')
		$form.find(`[name="pass_requirement"]`).val(`{{ $course->pass_requirement }}`).trigger('change')

		setTimeout(() => {
			$form.find(`[name="video_link"]`).val(`{{ $course->video_link }}`).trigger('change')
		}, 500)
...
<script type="text/html" id="videoSourceLinkTemplate">
	<div class="form-group">
		...
		<input type="text" name="video_link" class="form-control" placeholder="https://youtube.com/xxxxx" required>

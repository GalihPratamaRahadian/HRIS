...
@if(UserPermission::check('leave_submission', 'c'))
<a class="btn btn-primary" href="{{ route('admin.leave_submission.create') }}">
...
<div class="alert alert-primary">
	Ada {{ $pending }} pengajuan cuti menunggu persetujuan. <a href="{{ route('admin.leave_submission') }}?status=wait"> Klik disini </a> untuk melihat.
</div>
@endif
...
<script type="text/javascript">
	$(function(){
		...
		$('#dataTable').DataTable({
			...
			autoWidth : false,
			ajax : {
				url : "{{ route('admin.leave_submission') }}"
			},
			columns : [
				{
					data : 'created_at',
			...
			preDrawCallback : settings => {
				...
				settings.ajax.url = `{{ route('admin.leave_submission') }}?status=${status}`;
			},
			order: [[ '0','desc']]
		})

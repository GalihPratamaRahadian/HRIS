			...
			<div class="card-body">
				...
				<form id="mainForm">
					...
					<div class="form-group">
						...
						<input type="text" name="reason" class="form-control" placeholder="Alasan Sakit" value="{{ $sickReason->reason }}" required>
...
<script type="text/javascript">
	$(function(){
		...
		$form.on('submit', function(e){
			...
			ajaxSetup();
			$.ajax({
				url : `{{ route('admin.sick_reason.update', $sickReason->id) }}`,
				method : 'put',
				data : formData,
				dataType : 'json',
			})
		...
		$form.find(`[name="leave_type"]`).val(`{{ $sickReason->leave_type }}`)

		$form.find(`[name="is_using_max_duration"]`).val(`{{ $sickReason->is_using_max_duration }}`).trigger('change')
		setTimeout(() => {
			$form.find(`[name="max_duration"]`).val(`{{ $sickReason->max_duration }}`)
		}, 500)

		$form.find(`[name="is_counted_present"]`).val(`{{ $sickReason->is_counted_present }}`)
		$form.find(`[name="is_required_file"]`).val(`{{ $sickReason->is_required_file }}`)

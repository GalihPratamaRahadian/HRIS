...
<div class="row">
	...
	<div class="col-md-4">
		...
		@if($participant->isHavePassed())
		<a class="btn btn-primary btn-block" href="{{ route('employee.elearning.download_certificate', $course->id) }}" target="_blank">
...
<script type="text/javascript">
	...
	$(function(){
		...
		const renderEvent = () => {
			...
			$('.comment-container').find('.reply').on('click', function(){
				...
				$replyForm.on('submit', function(e){
					...
					ajaxSetup()
					$.ajax({
						url: `{{ route('employee.elearning.comment.create') }}`,
						method: 'post',
						dataType: 'json',
						data: formData
					})
		...
		const videoPassed = () => {
			ajaxSetup()
			$.ajax({
				url: `{{ route('employee.elearning.pass_video', $course->id) }}`,
				method: 'post',
				dataType: 'json',
			})
			.done(response => {
		...
		const saveSecondsPassed = (seconds, duration) => {
			if(!saveSecondsPassedRequest) {
				...
				ajaxSetup()
				$.ajax({
					url: `{{ route('employee.elearning.save_seconds_passed', $course->id) }}`,
					method: 'post',
					dataType: 'json',
					data: {
		...
		setInterval(() => {
			...
			saveSecondsPassed(videoPlayer.currentTime, videoPlayer.duration)
		}, 2000)
		@endif

		$('.exam-btn').on('click', function(){
			window.location.href = `{{ route('employee.elearning.exam', $course->id) }}`
		});

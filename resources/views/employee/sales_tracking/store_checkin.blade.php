						...
						<div class="card-body">
							<div class="support-pane">
								...
								<div class="form-group">
									<label> Foto Selfie Di Dalam Toko {!! Template::required() !!} </label> <br>
								...
								<div class="form-group">
									<label> Apakah Toko Melakukan Pembelian? {!! Template::required() !!}</label>
									<select class="form-control" name="purchase" required>
										<option selected disabled> - Pilih - </option>
										<option value="yes"> Ya </option>
										<option value="yes"> Tidak </option>
									</select>
								</div>
	...
	$(function(){
		...
		const formSubmit = () => {
			$('#mainForm').on('submit', function(e){
				...
				ajaxSetup();
				$.ajax({
					url : `{{ route('emp.sales_tracking.save_store_check_in', $store->id) }}`,
					method : 'post',
					data : formData,
					dataType : 'json',
				})

					...
					<div class="card support-pane-card">
						<div class="card-body">
							<div class="support-pane">
								...
								<div class="form-group">
									<input type="hidden" name="check_day_photo_base64">
...
<script type="text/javascript">
	$(function(){
		...
		const capture = () => {
			...
			$('[name="check_day_photo_base64"]').val(dataUrl);
		...
		const buttonEvent = () => {
			...
			$('.removeBtn').on('click', function(){
				...
				$('#cameraVideo').show();
				enable($('.captureBtn'));
				$('[name="check_day_photo_base64"]').val('');
				$(this).hide();
				$('.downloadBtn').hide();
				$('.stopBtn').show();
				$('.captureBtn').show();
		...
		const validateInput = () => {
			let photo = $('[name="check_day_photo_base64"]').val(),
		...
		const formSubmit = () => {
			$('#mainForm').on('submit', function(e){
				...
				ajaxSetup();
				$.ajax({
					url : `{{ route('employee.tracking.save_check_day', $trackingLocation->id) }}`,
					method : 'post',
					data : formData,
					dataType : 'json',
					contentType: false,

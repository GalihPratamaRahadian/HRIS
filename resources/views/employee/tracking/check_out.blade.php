					...
					<div class="card support-pane-card">
						<div class="card-body">
							<div class="support-pane">
								...
								<div class="form-group">
									<input type="hidden" name="check_out_photo_base64">
									<small class="invalid-feedback"></small>
									<input type="hidden" name="id_tracking_location" value="{{ $trackingLocation->id }}">
								</div>

								<div class="form-group">
									<label> Upload Bukti Penerimaan Barang {!! Template::required() !!} </label>
									<input type="file" name="file_upload_good_receipt" class="form-control" required>
									<small class="d-block my-1"> Hanya menerima format .jpeg, .jpg, .png, .pdf </small>
								...
								<button type="submit" class="btn btn-danger btn-block mt-2">
									Check Out <i class="mdi mdi-logout"></i>
								</button>
...
<script type="text/javascript">
	$(function(){
		...
		const capture = () => {
			...
			$('[name="check_out_photo_base64"]').val(dataUrl);
		...
		const buttonEvent = () => {
			...
			$('.removeBtn').on('click', function(){
				...
				$('#cameraVideo').show();
				enable($('.captureBtn'));
				$('[name="check_out_photo_base64"]').val('');
				$(this).hide();
				$('.downloadBtn').hide();
				$('.stopBtn').show();
				$('.captureBtn').show();
		...
		const validateInput = () => {
			let photo = $('[name="check_out_photo_base64"]').val(),
		...
		const formSubmit = () => {
			$('#mainForm').on('submit', function(e){
				...
				ajaxSetup();
				$.ajax({
					url : `{{ route('employee.tracking.save_check_out', $trackingLocation->id) }}`,
					method : 'post',
					data : formData,
					dataType : 'json',
					contentType: false,
